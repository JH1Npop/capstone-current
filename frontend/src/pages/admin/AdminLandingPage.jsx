...
import { PUBLIC_SITE_MANAGE_CAPABILITIES, hasAnyCapability, isSuperadmin } from '../../rbac';
...
export default function AdminLandingPage() {
  const { user } = useAuth();
  const canManageLandingPage = isSuperadmin(user) || hasAnyCapability(user, PUBLIC_SITE_MANAGE_CAPABILITIES);
  ...
  const save = async () => {
    if (!canManageLandingPage) {
      setMessage({ type: 'error', text: 'You have view-only access. Ask a superadmin for the Manage public site capability.' });
      return;
    }
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold text-slate-950">Landing Page Editor</h1>
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={save} disabled={loading || saving || !canManageLandingPage} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50">
          ...
          {!canManageLandingPage ? <div className="mb-5 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">You have view-only access. Preview is available, but publishing requires the <strong>Manage public site</strong> capability.</div> : null}

          <fieldset disabled={loading || !canManageLandingPage} className="contents">
                            ...
                            <div className="flex-1">
                              <div className="flex flex-wrap gap-2">
                                <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                                  ...
                                  <input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploadingPromotionId === promotion.id} className="hidden" onChange={(event) => { uploadPromotionImage(promotion.id, event.target.files?.[0]); event.target.value = ''; }} />
                                </label>
