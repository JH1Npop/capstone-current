...
import { deleteSolarEstimate, fetchSolarEstimates } from '../../api/api';
...
export default function ClientSolarEstimates() {
  ...
  const removeEstimate = async (estimate) => {
    if (!window.confirm(`Delete draft estimate #${estimate.id}?`)) return;
    try {
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Solar estimates</h1>
            <p className="mt-1 text-sm text-slate-500">Review saved calculations and request an on-site assessment.</p>
          </div>
          <button onClick={() => navigate('/solar-calculator#solar-calculator')} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-bold text-slate-950 hover:bg-amber-300">
            ...
            return (
              <article key={estimate.id} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-amber-600">Estimate #{estimate.id}</p>
